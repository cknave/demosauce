...
#define CXT_PARA struct _CXT * cxt 
...
#define linprebuf (cxt->linprebuf)
#define linpre (cxt->linpre)
#define lstepbuf (cxt->lstepbuf) 
#define lstep (cxt->lstep)
#define loutbuf (cxt->loutbuf)
#define lout (cxt->lout)
#define rinprebuf (cxt->rinprebuf)
#define rinpre (cxt->rinpre)                    
#define rstepbuf (cxt->rstepbuf)
#define rstep (cxt->rstep)
#define routbuf (cxt->routbuf)
#define rout (cxt->rout)
#define sampleWindow (cxt->sampleWindow)            
#define totsamp (cxt->totsamp)
#define lsum (cxt->lsum)
#define rsum (cxt->rsum)
#define freqindex (cxt->freqindex)
#define first (cxt->first)
#define AA (cxt->A)
#define BB (cxt->B)
...
int
ResetSampleFrequency (CXT_PARA, long samplefreq) {
    int  i;
...
int
InitGainAnalysis (CXT_PARA, long samplefreq)
{
    if (ResetSampleFrequency(cxt, samplefreq) != INIT_GAIN_ANALYSIS_OK) {
...
int
AnalyzeSamples (CXT_PARA, const Float_t* left_samples, const Float_t* right_samples, size_t num_samples, int num_channels )
{
...
static Float_t
analyzeResult (CXT_PARA, Uint32_t* Array, size_t len )
{
    Uint32_t  elems;
    Int32_t   upper;
    size_t    i;
...
Float_t
GetTitleGain (CXT_PARA)
{
    Float_t  retval;
    int    i;

    retval = analyzeResult (cxt, AA, sizeof(AA)/sizeof(*AA) );
...
Float_t
GetAlbumGain (CXT_PARA)
{
    return analyzeResult (cxt, BB, sizeof(BB)/sizeof(*BB) );
}

Context_t * NewAnalyzeContext( void )
{
	Context_t * cxt = malloc(sizeof(Context_t));
